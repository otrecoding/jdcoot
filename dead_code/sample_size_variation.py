...
def Sample_Size_Variation(Sample_sizes, Data=None, Monte_Carlo=1, Multi_Variations=True, algo='both',
    ...
    if Multi_Variations:
            ...
            for i in range(Monte_Carlo):
                ...
                for samp_s in Sample_sizes:
                    for samp_t in Sample_sizes:
                        scenario = data_generator(samp_s, samp_t, d_Source, d_Target, mean_X_Source, mean_X_Target,
                                                  mean_Y_Source,
                                                  mean_Y_Target, rho_source_generation, rho_source_non_generation,
                                                  rho_target_generation, rho_target_non_generation, Sparse_Rate,
                                                  Odds_Ratio_Source, Odds_Ratio_Target, R2_Source, R2_Target,
        ...
        graph_multi(Data, algo, Sample_sizes, Objective_Variable, "Sample size variation impact",
                    "$n^T=n^S=$Sample Size", "sample size")
        ...
        if (type(Data) is tuple and len(Data) != 2):
            ...
            for i in range(Monte_Carlo):
                ...
                test_ref = np.array([])
                for samp_s in Sample_sizes:
                    scenario = data_generator(samp_s, samp_s, d_Source, d_Target, mean_X_Source, mean_X_Target,
                                              mean_Y_Source,
                                              mean_Y_Target, rho_source_generation, rho_source_non_generation,
                                              rho_target_generation, rho_target_non_generation, Sparse_Rate,
                                              Odds_Ratio_Source, Odds_Ratio_Target, R2_Source, R2_Target,
        ...
        if Store_in:
            Store((
                  ...
                  test_MC_ref_perf, test_bp_coot, test_bp_jdcoot), "Sample_Size_Variation")
        return (MC_coot_perf,
                MC_jdcoot_perf,
                MC_ref_perf,
                bp_coot,
