...
def Proportion_Of_Observed_Covariates_Variation(Observed_Covariates_Proportion, Data=None, Monte_Carlo=1,
                                                ...
                                                , n_Source=1000, n_Target=1000, d_Source=100, d_Target=100,
                                                mean_X_Source=np.zeros(100),
                                                mean_X_Target=np.zeros(100),
                                                mean_Y_Source=0, mean_Y_Target=0, rho_source_generation=0.7,
                                                rho_source_non_generation=0.2,
                                                rho_target_generation=0.7, rho_target_non_generation=0.2,
                                                Sparse_Rate=0.75,
                                                Odds_Ratio_Source=0.5, Odds_Ratio_Target=0.5, R2_Source=0.6,
                                                R2_Target=0.6, Indexes_Chosen_For_Generation=INDEX_GENERATION):
    """
    ...
    test_Data_ = data_generator(n_Source, n_Target, d_Source, d_Target, mean_X_Source, mean_X_Target, mean_Y_Source,
                                ...
                                Observed_Covariates_Proportion_Source=1, Observed_Covariates_Proportion_Target=1,
                                Indexes_Chosen_For_Generation=Indexes_Chosen_For_Generation, Poisson=Poisson)
    ...
    if Multi_Variations:
            ...
            for i in range(Monte_Carlo):
                ...
                for samp_s in Sample_sizes:
                    for samp_t in Sample_sizes:
                        scenario = data_generator(n_Source, n_Target, d_Source, d_Target, mean_X_Source, mean_X_Target,
                                                  mean_Y_Source,
                                                  mean_Y_Target, rho_source_generation, rho_source_non_generation,
                                                  rho_target_generation, rho_target_non_generation, Sparse_Rate,
                                                  Odds_Ratio_Source, Odds_Ratio_Target, R2_Source, R2_Target,
                                                  Observed_Covariates_Proportion_Source=samp_s,
                                                  Observed_Covariates_Proportion_Target=samp_t,
                                                  Indexes_Chosen_For_Generation=Indexes_Chosen_For_Generation,
                                                  Poisson=Poisson)

                        perfs = Performance(scenario, test_Data_, alpha=alpha, algo=algo,
        ...
        graph_multi(Data, algo, Sample_sizes, Objective_Variable, "Proportion of observed covariables variation impact",
                    "$n^T=n^S=$Proportion of \n observed covariables", "Proportion of \n observed covariables")
            ...
            for i in range(Monte_Carlo):
                ...
                for samp_s in Sample_sizes:
                    scenario = data_generator(n_Source, n_Target, d_Source, d_Target, mean_X_Source, mean_X_Target,
                                              ...
                                              Odds_Ratio_Source, Odds_Ratio_Target, R2_Source, R2_Target,
                                              Observed_Covariates_Proportion_Source=samp_s,
                                              Observed_Covariates_Proportion_Target=samp_s,
                                              Indexes_Chosen_For_Generation=Indexes_Chosen_For_Generation,
                                              Poisson=Poisson)

                    perfs = Performance(scenario, test_Data_, alpha=alpha, algo=algo, type_supervision=type_supervision,
        ...
        if Store_in:
            Store((
                  ...
                  test_MC_ref_perf, test_bp_coot, test_bp_jdcoot), "Proportion_Of_Observed_Covariates_Variation")
        return (MC_coot_perf,
                MC_jdcoot_perf,
                MC_ref_perf,
                bp_coot,
