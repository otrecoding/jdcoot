...
def discrete_partial_reference(
    source,
    target,
    test_source,
    test_target,
    ...
    **kwargs,
    ...
    z_source_train = one_hot(
        source_train.Z.values[:, np.newaxis], source_levels_train
    ).astype(np.float64)

    x_target_train = target_train.loc[:, xcolumns(target)].values
    z_target_train = one_hot(
        target_train.Z.values[:, np.newaxis], target_levels_train
    ).astype(np.float64)
    ...
    clf_source = discrete_classifier(
        source, "relu", "softmax", len(source_levels_train)
    )
    clf_target = discrete_classifier(
        target, "relu", "softmax", len(target_levels_train)
    )
    clf_target.fit(
        x_target_train, z_target_train, batch_size=batch_size, epochs=10, verbose=0
    )
    clf_source.fit(
        x_source_train, z_source_train, batch_size=batch_size, epochs=10, verbose=0
    )
