...
def Store(Data, name):
    ...
    Input : Data (tuple) : tuple of 10 or 14 elements with respectively : 
    For Multivariation Scenarios (Heatmap) : 14 elements :  
    ...
    if not os.path.isdir(os.path.join(os.getcwd(),"Stored_Data")):
        os.mkdir(os.path.join(os.getcwd(),"Stored_Data"))

    if os.path.isdir(os.path.join(os.getcwd(),"Stored_Data", name)):
        for i in range(1, 1000):
            if not os.path.isdir(os.path.join(os.getcwd(), "Stored_Data", name + "_" + str(i))):
                name = name + "_" + str(i)
                print(f"Existing save, creating {name}")
                break

    os.mkdir(os.path.join(os.getcwd(), "Stored_Data", name))
