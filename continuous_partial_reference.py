...
def continuous_partial_reference(S, T, S_test, T_test):
    ...
    perf_pure = (sum((z_test - T_test.loc[:, 'Y']) ** 2) + sum((z_test2 - S_test.loc[:, 'Y']) ** 2)) / (
    ...
    perf_test = (sum((z_test - T.loc[ np.setdiff1d(np.arange(0, np.shape(T)[0]), y_labelled_target), 'Z']) ** 2) + sum(
...
if __name__ == "__main__":
    ...
    perf_pure, perf_test = continuous_partial_reference(S, T, S_test, T_test)

    print("Pure Performance Reference : {} ".format(perf_pure))
    print("Test Performance Reference : {} ".format(perf_test))
